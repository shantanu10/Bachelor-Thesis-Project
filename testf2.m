...
xlabel 'Hand movement1';
ylabel 'Hand movement2';
        ...
        for j=1:N
            ...
            if lightn(i)<lightn(j)
             ...
             % update centriod
                ...
                for ii = 1:S
                    for jj = 1:K
                        num = sum((sam1(ii,:)-cen(jj,:)).^2);
                        den = 0; frac = 0.00;
                        for kk = 1:K
                            den = sum((sam1(ii,:)-cen(kk,:)).^2);
                            frac = frac + (num/den)^(2.0/(m-1));
                        end    
                        u(ii,jj) = 1.00/frac; 
                    end
                end
